#include "process_eq_lognormal_z.cuh"
namespace prcr
{
    
    __host__ __device__
    Process_eq_lognormal_z::Process_eq_lognormal_z(rnd::MyRandom * gnr)
        :Process(gnr)
    {
    }

    __host__ __device__ double
    Process_eq_lognormal_z::Get_new_eq_price(
    ...
    __host__ __device__ double
    Process_eq_lognormal_z::compute_eq_price_exact(
                        double eq_price,
                        double r,
                        double div_yield,
                        double delta_t,
