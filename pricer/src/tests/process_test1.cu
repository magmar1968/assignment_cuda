...
#include "../lib/equity_lib/equity_prices.cuh"
...
#include "../lib/contract_option_lib/contract_eq_option.cuh"
    ...
    Result* results)
{
    ...
    prcr::Contract_eq_option &cast_contr_opt = static_cast<prcr::Contract_eq_option&> (*contr_opt);

    results[index].opt_price = 0;
    results[index].error = 0;

    for (int i = 0; i < PPT; i++)
    {
	prcr::Equity_prices* eq_price = new prcr::Equity_prices(cast_contr_opt.Get_schedule()->Get_t(0),
                                        			cast_contr_opt.Get_eq_prices()->Get_eq_price(),
                                                                cast_contr_opt.Get_eq_prices()->Get_eq_description());
        /*for (int j = 0; j < contr_opt->Get_schedule()->Get_dim()-1; j++)
        {
	    prcr::Equity_prices* old_price = eq_price;
            double w = process->Get_random_gaussian();
            double delta_t = contr_opt->Get_schedule()->Get_t(j + 1) - contr_opt->Get_schedule()->Get_t(j);
            eq_price = process->Get_new_prices(eq_price, w, delta_t);
            delete(old_price);
        }*/
        //results[index].opt_price += eq_price->Get_eq_price().get_number();
        //results[index].error += eq_price->Get_eq_price().get_number()*eq_price->Get_eq_price().get_number();
	delete(eq_price);
    }
    
    //results[index].opt_price /= PPT;
    delete(gnr_in);
    delete(process);
