...
int main(int argc, char **argv)
{
    ...
    {
        seeds[inc] = rnd::genSeed(true);       //same seeds for gpu and cpu --> same results? let's check!
    }
    ...
    if (dev.CPU == true)
    {
        ...
        double* path_CPU = new double[NEQ]; //contiene i prezzi delle equities allo step finale
        ...
        if (dev.GPU == true)           //se -gpu setted
        {
            ...
            cudaStatus = cudaMemcpy(path_GPU, dev_path, NEQ * sizeof(double), cudaMemcpyDeviceToHost);  //memcpy path_out da device
            ...
            for (int k = 0; k < NEQ; k++)
            {
		    minimum = min(path_CPU[k], path_GPU[k]);  //minimo tra prezzo della stessa equity al final step tra gpu e cpu
            if (abs(path_CPU[k] - path_GPU[k])>0.000001*minimum) 
                err_msg += pow(2, k + 1);  //se i valori si discostano troppo tra di loro ("troppo definito da quale tra i due è min)
