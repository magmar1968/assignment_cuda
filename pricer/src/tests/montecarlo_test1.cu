...
__global__ void kernel_mc(uint* dev_seeds,
    ...
    Result* dev_results)
{
    udb start_price;                              //definiamo oggetti dentro a kernel
    start_price = dev_prices->start_price;
    ...
    descr ->Set_isin_code(dev_descr->isin_code);
    descr ->Set_name(dev_descr->name);
    descr ->Set_currency(dev_descr->currency);
    descr ->Set_dividend_yield(dev_descr->div_yield);
    descr ->Set_yc(&yc);
    descr ->Set_vol_surface(&vol);
     

    Equity_prices starting_point_in = Equity_prices(start_time,start_price, descr);
    ...
    Result* results)
{
    ...
    Process_eq_lognormal process = Process_eq_lognormal(&gnr_in);
...
int main(int argc, char** argv)
{
    ...
    if (dev.CPU == true)
    {
        ...
        Equity_description* descr  = new Equity_description;
        descr->Set_isin_code(dscrp_args->isin_code);
        descr->Set_name(dscrp_args->name);
        descr->Set_currency(dscrp_args->currency);
        descr->Set_dividend_yield(dscrp_args->div_yield);
        descr->Set_yc(yc);
        descr->Set_vol_surface(vol);
        

        Equity_prices* starting_point_in = new Equity_prices(prices_args->start_time, (prices_args->start_price), descr);
