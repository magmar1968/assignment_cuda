...
// Strutture dati che verrano passate al kernel:
...
__global__ void kernel_mc(uint* dev_seeds,
    ...
    double* dev_results,
    int* cuda_bool)
{                                                                               //copia oggetti da argometni di kernel
...
int main(int argc, char** argv)
{
    ...
    {
        dscrp_args->vol[i] = 0.0;
    }
    strcpy(dscrp_args->isin_code, "codice isin");
    ...
    if (dev.CPU == true)
    {
        ...
        Equity_prices* starting_point_in = new Equity_prices(prices_args->start_time, prices_args->start_prices, descr);
