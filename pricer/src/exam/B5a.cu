...
run_device(prcr::Pricer_args* prcr_args, Result* host_results, uint* host_seeds)
{
   ...
   // std::cout << "Using device: " << dev_label << std::endl;
    ...
    cudaFree(dev_seeds);
    double tme = gpu_timer.GetDeltamsTime();
    return tme;
}
...
{   
    ...
    for(size_t NBLOCKS = 10 ; NBLOCKS <601 ; NBLOCKS +=10)
    {
        ...
        std::string filename = path_infile + "infile_B5a.txt";
        ...
        prcr_args->schedule_args.dim = m_values[i];
        // gen seeds 
        srand(time(NULL));
        ...
        filename_output = "./data/out_B5_GPU_m" + std::to_string(m_values[i]) + ".txt";
        std::ofstream fs;
        ...
        if (CPU == true)
        {
            ...
            double cpu_time = timer_cpu.GetDeltamsTime();
